...
from transformers import AutoModelForCTC, AutoProcessor, Wav2Vec2Processor, HubertForCTC
...
class ASR:
    def __init__(self, opt):
        ...
        self.sample_rate = opt.sample_rate # 16000
        self.chunk = self.sample_rate // self.fps # 320 samples per chunk (20ms * 16000 / 1000)
        self.mode = 'live' if opt.asr_wav == '' else 'file'

        if 'esperanto' in self.opt.asr_model:
            self.audio_dim = 3503 # [NOTE] 3503
        ...
        if self.mode == 'file':
            ...
            # print(self.file_stream.shape) 2727073
            # exit()
        else:
        ...
        # self.processor = Wav2Vec2Processor.from_pretrained("/mnt/home/my-blendshape-nerf/models--facebook--hubert-large-ls960-ft/snapshots/ece5fabbf034c1073acae96d5401b25be96709d8") # (opt.asr_model)
        # self.model = HubertForCTC.from_pretrained("/mnt/home/my-blendshape-nerf/models--facebook--hubert-large-ls960-ft/snapshots/ece5fabbf034c1073acae96d5401b25be96709d8").to(self.device)
        # self.processor = AutoProcessor.from_pretrained("/mnt/home/RAD-NeRF/models--cpierse--wav2vec2-large-xlsr-53-esperanto/snapshots/3ff4b48db24592341ea7cc8930da9a1b172b4930") # (opt.asr_model)
        # self.model = AutoModelForCTC.from_pretrained("/mnt/home/RAD-NeRF/models--cpierse--wav2vec2-large-xlsr-53-esperanto/snapshots/3ff4b48db24592341ea7cc8930da9a1b172b4930").to(self.device)
        self.processor = AutoProcessor.from_pretrained("/mnt/home/my-blendshape-nerf/models--jonatasgrosman--wav2vec2-large-xlsr-53-chinese-zh-cn/snapshots/99ccb2737be22b8bb50dcfcc39ad4d567fb90cfd") # (opt.asr_model)
        self.model = AutoModelForCTC.from_pretrained("/mnt/home/my-blendshape-nerf/models--jonatasgrosman--wav2vec2-large-xlsr-53-chinese-zh-cn/snapshots/99ccb2737be22b8bb50dcfcc39ad4d567fb90cfd").to(self.device)

        # print(self.model)
        
        # 通过hook获取特征
        layer_name =  'wav2vec2.encoder.layers.23.final_layer_norm' # 'hubert.encoder.layers.23.final_layer_norm' #
    ...
    def run_step(self):
        ...
        if frame is None:
            ...
            # 每次左边stride 10, 右边stride 10, 中间50context, 266*70个采样点
            if len(self.frames) < self.stride_left_size + self.context_size + self.stride_right_size:
                return

        inputs = np.concatenate(self.frames) # [N * chunk]

        # print(inputs.shape) # 266*70

        # discard the old part to save memoryself.frames[
        if not self.terminated:
            # skip掉了context_size个266个采样点
    ...
    def frame_to_text(self, frame):
        ...
        # 19200采样率 60fps, 每秒320个采样点，和16000采样率, 50fps, 每秒320个采样点应该等价
        inputs = self.processor(frame, sampling_rate=16000, return_tensors="pt", padding=True)
        # print(frame.shape) 
        # print(inputs.input_values.shape) 1, N*266

        with torch.no_grad():
            result = self.model(inputs.input_values.to(self.device))
            logits = result.logits # [1, N - 1, 32]
            # print(inputs.input_values.to(self.device).shape)
            # print(self.features_out_hook[-1].shape)
            # print(logits.shape)
            feature = self.features_out_hook[-1] 
        ...
        return logits[0], predicted_ids[0], transcription # [N,] # [NOTE] logits换成feature
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample_rate', type=int, default=19200)
