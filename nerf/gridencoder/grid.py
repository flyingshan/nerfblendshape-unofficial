...
class ExpGridEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=None, gridtype='hash', align_corners=False, interpolation='linear', basis_num=8):
        super().__init__()
        ...
        self.basis_num = basis_num # plus mean shape
        ...
        self.embeddings_mean = nn.Parameter(torch.empty(1, offset, level_dim), requires_grad=True)
        self.embeddings = nn.Parameter(torch.empty(self.basis_num-1 , offset, level_dim), requires_grad=True)
    ...
    def forward(self, inputs, exp, bound=1):
        ...
        current_embeddings = current_embeddings.reshape([self.embeddings.shape[-2],self.embeddings.shape[-1]]) 
        # print(self.embeddings_mean.shape)
        # print(self.embeddings.shape)
        # print(current_embeddings.shape)
        # ([1, 903480, 4])
        # torch.Size([55, 903480, 4])
        # torch.Size([903480, 4])
        outputs = grid_encode(inputs, current_embeddings, self.offsets, self.per_level_scale, self.base_resolution, inputs.requires_grad, self.gridtype_id, self.align_corners, self.interp_id)
